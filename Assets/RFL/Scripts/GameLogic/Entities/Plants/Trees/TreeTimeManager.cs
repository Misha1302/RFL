namespace RFL.Scripts.GameLogic.Entities.Plants.Trees
{
    ...
    public class TreeTimeManager : MonoBeh
    {
        ...
        public void Init(double startTotalTime)
        {
            ...
            _creatorService.Create<TimeEvent>().Init(_startTotalTime).OnTimeCome +=
                () => OnTimeToPhase(TreePhaseType.Phase1);

            _creatorService.Create<TimeEvent>().Init(_startTotalTime + 5d).OnTimeCome +=
                () => OnTimeToPhase(TreePhaseType.Phase2);

            _creatorService.Create<TimeEvent>().Init(_startTotalTime + 10d).OnTimeCome +=
